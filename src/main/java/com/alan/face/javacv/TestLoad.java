...
public class TestLoad {
     ...
     * @date 2019年1月25日
     */
    public static void testLoadImg() throws URISyntaxException {
        //读取原始图片
        File imageF = new File(TestLoad.class.getResource("/pic/zxc1.jpg").toURI());
        Mat image = imread(imageF.getAbsolutePath());
        if (image.empty()) {
...
//        imshow("显示原始图像", image);
        ...
        //加载检测器
        File scade = new File(TestLoad.class.getResource("/detection/haarcascade_frontalface_alt.xml").toURI());
        CascadeClassifier face_cascade = new CascadeClassifier(scade.getAbsolutePath());//初始化人脸检测器
    ...
    public static void detectFace() throws Exception  {
        ...
        //加载检测器
        File scade = new File(TestLoad.class.getResource("/detection/haarcascade_frontalface_alt.xml").toURI());
        File eyeScade = new File(TestLoad.class.getResource("/detection/haarcascade_eye_tree_eyeglasses.xml").toURI());
        CascadeClassifier face_cascade = new CascadeClassifier(scade.getAbsolutePath());//初始化人脸检测器
        CascadeClassifier eye_cascade = new CascadeClassifier(eyeScade.getAbsolutePath());//初始化眼部检测器
        //定义人脸集合，矩形集合
        ...
        while (true) {
            ...
            for (int i = 0; i < eyes.size(); i++) {
                Rect eye = eyes.get(i);
                opencv_imgproc.rectangle(mat, eye, Scalar.GREEN, 1, 8, 0);
            }
     ...
     * @throws Exception
     */
    public static List<org.opencv.core.Mat> getFaces(String path) throws  Exception {
        List<org.opencv.core.Mat> faces = new ArrayList<>();
        //加载检测器
        File scade = new File(TestLoad.class.getResource("/detection/haarcascade_frontalface_alt.xml").toURI());
        org.opencv.objdetect.CascadeClassifier face_cascade = new org.opencv.objdetect.CascadeClassifier();
        //初始化人脸检测器
        face_cascade.load(scade.getAbsolutePath());
        org.opencv.core.Mat image = Imgcodecs.imread(path);
        MatOfRect vector = new MatOfRect();
        //当前帧图片进行灰度+直方均衡
        org.opencv.core.Mat videoMatGray = new org.opencv.core.Mat();
        Imgproc.cvtColor(image, videoMatGray, Imgproc.COLOR_BGRA2GRAY);
        Imgproc.equalizeHist(videoMatGray, videoMatGray);

        //使用检测器进行检测，把结果放进集合中
        face_cascade.detectMultiScale(videoMatGray, vector);
        vector.toList().stream().forEach(e->{
            org.opencv.core.Mat temp =   new org.opencv.core.Mat(image, e);
            Imgcodecs.imwrite("c://test"+Math.random()*2000+".jpg",temp);
            faces.add(temp);
        });
        return faces;
    }
     ...
     * @date 2019年1月26日
     */
    public static double CmpPic(org.opencv.core.Mat src, org.opencv.core.Mat des) {
        System.out.println("\n==========直方图比较==========");
        //自定义阈值
        //相关性阈值，应大于多少，越接近1表示越像，最大为1
        double HISTCMP_CORREL_THRESHOLD = 0.6;
        //卡方阈值，应小于多少，越接近0表示越像
        double HISTCMP_CHISQR_THRESHOLD = 2;
        //交叉阈值，应大于多少，数值越大表示越像
        double HISTCMP_INTERSECT_THRESHOLD = 1.2;
        //巴氏距离阈值，应小于多少，越接近0表示越像
        double HISTCMP_BHATTACHARYYA_THRESHOLD = 0.4;
        ...
        try {
            ...
            Imgproc.cvtColor(des, hsv_des, Imgproc.COLOR_BGR2HSV);
...
//			detectFace();
//			testCamera();
//			testFace(new File(TestLoad.class.getResource("/pic/yc1.jpg").toURI()).getAbsolutePath());
            {
                System.load(new File(TestLoad.class.getResource("/opencv_java412.dll").toURI()).getAbsolutePath());
            }
            List<org.opencv.core.Mat> srcs = getFaces(new File(TestLoad.class.getResource("/pic/yc1.jpg").toURI()).getAbsolutePath());
            List<org.opencv.core.Mat> dess =getFaces(new File(TestLoad.class.getResource("/pic/zxc1.jpg").toURI()).getAbsolutePath());

            if(srcs.size()>0&&dess.size()>0)
                System.out.println(CmpPic(srcs.get(0), dess.get(0)));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
