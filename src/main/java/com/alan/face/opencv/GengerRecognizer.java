...
public class GengerRecognizer extends JPanel {
    private static Logger log = LoggerFactory.getLogger(GengerRecognizer.class);
    private static final String TAG = "GengerRecognizer";
    ...
    public static void main(String[] args) throws Exception{
        ...
        File proto = new File(GengerRecognizer.class.getResource("/assets/deploy_gender.prototxt").toURI());
        File weights = new File(GengerRecognizer.class.getResource("/assets/gender_net.caffemodel").toURI());
        genderNet = Dnn.readNetFromCaffe(proto.getAbsolutePath(), weights.getAbsolutePath());
        if (genderNet.empty()) {
            log.info(TAG, "Network loading failed");
        } else {
            log.info(TAG, "Network loading success");
        }
        mFaceDetector = new CascadeClassifier(new File(GengerRecognizer.class.getResource("/detection/lbpcascade_frontalface_improved.xml").toURI()).getAbsolutePath());
       openCamera();
    }

    private static String predictSex(Mat mRgba, Rect[] facesArray) {
        try {
            for (Rect face : facesArray) {
                Mat capturedFace = new Mat(mRgba, face);
                //Resizing pictures to resolution of Caffe model
                Imgproc.resize(capturedFace, capturedFace, new Size(250, 250));
            ...
            log.error(TAG, "Error processing sex", e);
        }
        return null;
    }
    ...
    public static void openCamera() throws  Exception {
        try{
            ...
            GengerRecognizer panel = new GengerRecognizer();
    ...
    private static Mat convert(Mat mRgba){
        ...
        if (facesArray.length == 1) {
            String sex = predictSex(mRgba, facesArray);
            log.info(TAG, "Sex is: " + sex);

            //The result of sex recognition
            for (Rect face : facesArray) {
                int posX = (int) Math.max(face.tl().x - 10, 0);
                int posY = (int) Math.max(face.tl().y - 10, 0);

                Imgproc.putText(mRgba, "Sex: " + sex, new Point(posX, posY), FONT_HERSHEY_TRIPLEX,
